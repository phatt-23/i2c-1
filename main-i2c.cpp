...
#if 1 // implemeted 23:12:54
        if((!g_but_PTC9 && !g_but_PTC12) || true) {
            separate();
            printf(">> BUTTONS:  1 0 0 1 \n");
            l_ack = rad.get_rds_status(l_rds, 13);
            if(!l_ack)
                printf("OK :: Getting RDS Status\n");
            if(l_rds[2] >> 7 == 1 || true) // valid input
            {
                printf("getting radio station name and alt freq\n");
                if((l_rds[6] >> 2) == 0) // group 0A
                {
                    i32 pos = (l_rds[7] & 0x03) * 2;
                    g_station_name[pos] = l_rds[10];
                    g_station_name[pos + 1] = l_rds[11];
                    // 1 -> 51
                    // 87.6 -> 107.9
                }
